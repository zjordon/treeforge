...
> 2026-08-30 修订：**站点级形态对齐 Browser-BC**——有界摘要（功能地图 + 站点通用知识，
> 8000 内「压缩措辞不丢主题」），逐任务序列归任务卡；增量 addendum 改「以旧卡为基线更新」。
> 由 localhost 15 任务实测教训驱动：原「按 capacity 每任务一节」+ 单卡 8000 导致旧任务
> 被直接丢弃（模型自造「历史操作序列」归档节、逐轮再压缩 = 传话游戏式内容流失）。

## 一、背景与动机

TreeForge 任务级 skill（录一遍任务 → 蒸馏 SOP → 文件注入）已闭环。蓝图路线二要的是
**站点级**：同一站点多任务累积蒸馏出「布局 / 菜单 / 功能地图 / 站点通用知识」（有界摘要），消
...
8. **一次蒸馏双产物，两次 LLM 调用、两套 prompt**：用户操作一次，管线内部跳 A
   ...
   |---|---|---|
   | sop_md spec | 有界站点摘要：功能地图 + 站点通用操作知识（≤8000，压缩不丢主题；2026-08-30 修订） | 现行单任务连贯流程叙事（无地图段） |
...
### S3 站点级产物形态（长在 _sop.md 里；2026-08-30 按 Browser-BC 修订）

`_DISTILL_PROMPT_TEMPLATE` 的 `sop_md` spec：**有界站点摘要**（不新增文件——
TreeWalker loader 固定读三件名）：

- **必写开头段 `## 站点功能地图（Site Function Map）`**：入口 URL 模式（含 SPA 路由）、
  主要菜单 / 功能区清单（每区一句话：在哪、通向什么任务）。跨任务累积。
- 之后 `## 站点通用操作知识`：**跨任务共性**only——共享导航路径、公共控件行为
  （筛选/网格/日期选择器）、适用于多类任务的重复操作模式；要点式精炼，
  **不是逐任务步骤剧本**（那些在 `tasks/<slug>/` 任务卡）。
- **有界**：整份 sop ≤~6000（硬上限 8000）；超限**压缩措辞，不丢主题**。
- capacities 降级为信息性清单（「本站已见任务类型」），与旧卡 capacities 并集
  （清单只增不减）；**不再**作分组结构。

【修订由头】原设计「按 capacity 每任务一节」在 localhost 15 任务实测中失败：
capacities 只含当前任务 + 分组指令 → 模型把旧任务挤进自造的「历史操作序列」归档节
并逐轮再压缩（传话游戏），16 轮后 15 个任务只剩 2 个序列。Browser-BC 的 8000 之所以
够用，是因为它的桶是 **domain::capacity**（知识天然有界）；站点级一张卡要有界，
靠「摘要 + 压缩不丢主题」而不是「逐任务堆积」。

测试：prompt 契约测试断言（`Site Function Map` / `站点通用操作知识` / `BOUNDED` /
反丢弃措辞在模板里；`GROUPED`/`典型操作序列` 已移除）；addendum 基线更新措辞；
`_PREV_SOP_BUDGET == 8000` 钉死；capacities 并集。
