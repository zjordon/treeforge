...
| 评论 Status/Type 筛选下拉 | 评论网格筛选行 | `id=reviewGrid_filter_status` / `id=reviewGrid_filter_type` | |
...
| 评论每页条数 / 当前页 / 翻页 | 网格底部 | `id=reviewGrid_page-limit` / `id=reviewGrid_page-current`, 可见文本"Next page" | page-limit 选项 20|30|50|100|200 |
...
| 订单评论输入/状态下拉 | 订单详情底部 Notes 区 | `id=history_comment` / `id=history_status` | 提交按钮靠 `title=Submit Comment` |