...
- 商品评论: `Marketing` → `Reviews` → `All Reviews` → `/admin/review/product/index/`（约 351 records；顶部 `New Review`(id=add)、Search、Reset Filter）。按字段筛选（如 Product name、Status=Pending）后 URL 变为 `/admin/review/product/index/filter/<base64>/internal_reviews//form_key/<随机>/`
- 评论详情/编辑: `/admin/review/product/edit/id/N/`。字段：Status `id=status_id`（Approved|Pending|Not Approved）、Nickname `id=nickname`、Summary `id=title`、正文 `id=detail`；按钮 Back(id=back)/Reset/Delete Review/Save Review(id=save_button)/Save and Next/Save and Previous/Next/Previous——按评论位置显示不同组合
...
- Scope 切换（Dashboard 头部）: `id=store-change-button`（All Store Views）+ Reload Data 按钮；`#store_switcher`/`#store_group_switcher`/`#website_switcher` change 事件触发 switchScope
...
- 两代网格：
  ...
  - 旧版网格（评论页 reviewGrid）：**无 Filters 按钮**，筛选行直接在表头下方；填完点顶部 `Search`（title=Search），清除用 `Reset Filter`（title=Reset Filter）。筛选字段 id 稳定：`reviewGrid_filter_name/sku/status/type/title/nickname/detail/review_id`、`reviewGrid_filter_massaction`、created_at[from]/[to]、`visible_in`。
...
- 下拉筛选（Status/Type/Store/visible_in、评论网格下拉）直接 `select_dropdown(index, value)`，无需先点击展开；`dropdown_options` 可先读选项。
...
- 分页：网格底部每页条数下拉（评论网格 `id=reviewGrid_page-limit`，20|30|50|100）、当前页 input（`id=reviewGrid_page-current`）、`of N`、Next/Previous page 按钮（title 指示方向）。
...
- 进入详情页：点行内 View/Edit 链接、点评论行本身（tr title 即编辑 URL），或直接 `navigate` 详情 URL；详情页长需多次 `scroll`；返回列表用顶部 `Back`（id=back）。
- 回到列表后筛选值保留（如 `reviewGrid_filter_name` 仍显示上次搜索的 Product name），可继续操作其余结果行；再搜索前视需要先 `Reset Filter`。